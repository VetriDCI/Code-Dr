...
- OUTPUT ONLY HTML CODE, no markdown, no explanation."""
    ...
    try:
        completion = client.chat.completions.create(
            ...
            reasoning_effort="low",  # gpt-oss models: keep reasoning short so content isn't truncated
        )
    except Exception as e:
        raise HTTPException(status_code=502, detail=f"Groq API error: {e}")

    message = completion.choices[0].message
    # gpt-oss (reasoning) models sometimes put the answer in `reasoning`
    # instead of `content` if content came back empty/None.
    html_code = (message.content or "").strip()
    if not html_code:
        html_code = (getattr(message, "reasoning", None) or "").strip()
    if not html_code:
        raise HTTPException(status_code=502, detail="Groq returned an empty response. Try again or simplify the prompt.")
